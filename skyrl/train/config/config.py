...
class OptimizerConfig(BaseConfig):
    ...
    inert when ``fsdp_config.cpu_offload=True`` or ``cpu_adam=True`` since both then keep the
    optimizer off the GPU by construction. Without colocation it can be preferable to leave
    optimizer state on GPU, avoiding both the offload cost and the extra CPU memory usage."""
...
class AlgorithmConfig(BaseConfig):
    ...
    "filter"/"replace" strategies already do this."""
